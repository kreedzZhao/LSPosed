...
//                                              "()[Ljava/lang/String;");
    ...
    ScopedLocalRef<jobject> Service::RequestApplicationBinderFromSystemServer(JNIEnv *env, const ScopedLocalRef<jobject> &system_server_binder) {
        ...
        // writeInt(getpid())
        // writeString("system")
        // writeStrongBinder(heart_beat_binder)
        ...
        if (res) {
            ...
            // readStrongBinder 获取到 IBinder 对象，可以通过这个对象与 服务端进行通信
            // 实际上就是 LSPosedService 对象
    ...
    /*
     * Request LSPosed dex file from LSPosedService
     */
    std::tuple<int, size_t> Service::RequestLSPDex(JNIEnv *env, const ScopedLocalRef<jobject> &binder) {
        Wrapper wrapper{env, this};
        // 是同一个服务端，所以使用同一个 binder 对象，通过不同的 code 来区分不同的请求
